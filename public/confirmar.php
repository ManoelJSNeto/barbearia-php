...
// recarrega status após possível POST
$stmt->execute([$token]);
$ag = $stmt->fetch();
...
<div style="padding-top:40px; padding-bottom:80px; max-width:520px;">
  ...
  <?php if ($ag['status'] === 'confirmado'): ?>
    ...
    <div class="booking-summary">
      <div class="booking-summary-row"><span class="bsr-label">Serviço</span><span class="bsr-value"><?= e($ag['item_nome']) ?></span></div>
      <div class="booking-summary-row"><span class="bsr-label">Barbeiro</span><span class="bsr-value"><?= e($ag['barbeiro_nome']) ?></span></div>
      <div class="booking-summary-row"><span class="bsr-label">Data</span><span class="bsr-value"><?= $dataHora->format('d/m/Y') ?></span></div>
      <div class="booking-summary-row">
        <span class="bsr-label">Horário</span>
        <span class="bsr-value" style="font-family:var(--font-mono); color:var(--accent); font-size:1.1rem;"><?= $dataHora->format('H:i') ?></span>
      </div>
      <div class="booking-summary-row">
        <span class="bsr-label">Valor</span>
        <span class="bsr-value" style="color:var(--accent); font-weight:600;">R$ <?= number_format((float)$ag['preco'], 2, ',', '.') ?></span>
      </div>
    </div>

    <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:20px;">
      <a class="btn" href="/agendar.php">Novo agendamento</a>
      <a class="btn btn--ghost" href="/cliente/dashboard.php">Meus agendamentos</a>
    </div>
  ...
  <?php elseif ($ag['status'] === 'pendente'): ?>
    ...
    <?php if ($agora <= $limite): ?>
      <form method="post" style="margin:16px 0 10px;">
        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
        <button class="btn btn--full" type="submit" style="padding:13px;">Confirmar presença agora</button>
      </form>
      <p style="font-size:12px; color:var(--muted);">
        Prazo para confirmar: até <?= $limite->format('d/m') ?> às <?= $limite->format('H:i') ?>.
      </p>
    <?php else: ?>
      <div class="flash flash--err" style="margin-top:16px;">O prazo de confirmação expirou.</div>
    <?php endif; ?>

    <div style="margin-top:14px;">
      <a class="btn btn--ghost btn--sm" href="/cliente/cancelar.php?token=<?= urlencode($token) ?>">Cancelar agendamento</a>
    </div>
  ...
  <?php elseif ($ag['status'] === 'cancelado'): ?>
    ...
    <p class="muted" style="margin-bottom:20px;"><?= e($ag['item_nome']) ?> com <?= e($ag['barbeiro_nome']) ?> — <?= $dataHora->format('d/m/Y H:i') ?></p>
  ...
  <?php else: ?>
    ...
    <p class="muted" style="margin-bottom:20px;">O prazo de confirmação passou.</p>
    <a class="btn" href="/agendar.php">Fazer novo agendamento</a>
  <?php endif; ?>
